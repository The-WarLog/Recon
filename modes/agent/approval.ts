...
for(const t of pending){
 ...
 for (const [p,action] of pathEntries){
    const sorted =action.sort((a,b)=> (a.timestamp.getTime()-b.timestamp.getTime()))
...
export async function runApprovalFollowUp(tracker: ActionTracker): Promise<boolean>{
  ...
  if (isCancel(choice) || choice === "cancel") {
    for (const a of pendingActions) tracker.updateStatus(a.id, "rejected", false);
    return false;
  }
  if(choice==="all"){
